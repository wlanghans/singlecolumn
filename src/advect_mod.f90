module advect_mod
   ...
   contains
      ...
      function advect_mass_fraction2(q,rho,limiter,zflux)
         ...
         ! Calculate the change in d
         advect_mass_fraction2(1) = -  0.5*rho(1)*w(  2     )*(q_i(2)-q_i(1)) * (1./adz(1)/dz)
         do k = 2,nzm-1
            kp = k + 1
            one_over_dz = (1./adz(k)/dz)
                  advect_mass_fraction2(k) = advect_mass_fraction2(k) + &
                    0.5 * rho(k)*(w(k )+w(kp))*(q_i(k ) - q_i(kp) ) * one_over_dz 
         end do
         advect_mass_fraction2(nzm) =  0.5*rho(nzm)*w( nzm)*(q_i(nzm)-q_i(nz)) * (1./adz(nzm)/dz)
      ...
      subroutine coriolis_add(SU, SV)
         ...
         !                        |---- To average rho
         !                        |
         SU = SU + rho(1:nzm) * fcor * (v(1:nzm)-vg(1:nzm))
         SV = SV - rho(1:nzm) * fcor * (u(1:nzm)-ug(1:nzm))
