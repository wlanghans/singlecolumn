Program Parallel_Run_Ensemble
  ...
  character(500) :: namehlp, casename,filein,fileinprev,rname
  ...
  integer :: i,j,k,i1,i2,i3,i4,i5,i6,lenc
  integer :: itmp(10)
  ...
  do i=1,N_local_runs
   ...
   pathin='"'//trim(path_in)//'"'
   rname='"'//trim(runname(i))//'"'

   lenc=len(namehlp)

   i_char=namehlp(lenc:lenc)
   rname=trim(rname)//trim(i_char)
   i6=ichar(i_char) 
   i_char=namehlp(lenc-3:lenc-3)
   rname=rname//i_char
   i5=ichar(i_char) 
   i_char=namehlp(lenc-6:lenc-6)
   i4=ichar(i_char) 
   i_char=namehlp(lenc-9:lenc-9)
   i3=ichar(i_char) 
   i_char=namehlp(lenc-12:lenc-12)
   i2=ichar(i_char) 
   i_char=namehlp(lenc-15:lenc-15)
   i1=ichar(i_char) 


   ! make sure previous run has completed yet
   if (i.gt.1.and.j.gt.0) then
   ...
   write(*,*) 'Simulation ', trim(runname(j)), ' was successful'
   command=" rm -rf "//trim(path_in)//"/"//trim(runname(j))//"/"//trim(fileinprev)
   status = system( trim(command))
   j=-1

   end if

   command="cd "//trim(path_in)//"/"//trim(runname(i))//"; /global/home/users/langhans/ncl-6.3.0/bin/ncl -Q "//trim(path_ncl)//"/getrmse_mm_rmseqtdthvd.ncl benchdir="//"'"//trim(path_ncl)//"'"//" casename="//"'"//trim(casename)//"'"//" runname="//"'"//trim(rname)//"'"//" pathin="//"'"//trim(pathin)//"'" 
   !command=trim(command)//"; rm -rf "//trim(path_in)//"/"//trim(runname(i))//"/"//trim(filein)

   filenow=trim(path_in)//"/"//trim(runname(i))//"/mmthvdqtdrmse4_thvd_"//trim(namehlp(1:5))
   ...
   if (.not.filenowexist) then
     ...
     open(78,file=trim(filenow),status='old',form='formatted')
        read (78,*) readparam(1:9)
     close(78)
     if (readparam(1).gt.5.) then
       command="rm -rf "//trim(filenow)
       status = system( trim(command))
       command="cd "//trim(path_in)//"/"//trim(runname(i))//"; /global/home/users/langhans/ncl-6.3.0/bin/ncl -Q "//trim(path_ncl)//"/getrmse_mm_rmseqtdthvd.ncl benchdir="//"'"//trim(path_ncl)//"'"//" casename="//"'"//trim(casename)//"'"//" runname="//"'"//trim(rname)//"'"//" pathin="//"'"//trim(pathin)//"'" 
       status = system( trim(command))
       j=i
