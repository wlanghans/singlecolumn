...
10   continue
  ...
  do n=run_start,run_end
    ...
    do ca=1,Ncases
      ...
      ! read rmsecase
      open(78,file=trim(path_in)//"/"//trim(runname)//"/"//'mmrmse_thvd_'//trim(casename(ca)),status='old',form='formatted')
      read (78,*) readparam(1:9)
      close(78)
      open(79,file=trim(path_in)//"/"//trim(runname)//"/"//'mmrmse_thvd_'//trim(casename(ca)),status='old',form='formatted')
  ...
  ! write netcdf file 
  fname_out =trim(path_in)//"/"//'RMSE_MM.nc' 
  write(*,*) fname_out
